var game = {
	...
	start: function(canvaId) {
		...
		d.forEach((enemyBallData) => {
			this.enemies.push(
				new EnemyBall(this.ctx, enemyBallData[0], enemyBallData[1], 'right', enemyBallData[2], enemyBallData[3])
			);
		});
		...
		this.interval = setInterval(
			function() {
				...
				if (
					(this.ball.y + this.ball.r <= 250 && this.ball.y + this.ball.r >= 210) ||
	...
	checkEggNest: function() {
		if (
			this.ball.y > 550 &&
			this.ball.y < 650 &&
			(this.ball.x < this.nest.x + this.nest.w + 2 && this.ball.x > this.nest.x - 2)
		) {
	...
	checkCollisionBall: function() {
		...
		this.board.forEach((wall) => {
			if (this.RectCircleColliding(this.ball, wall) === 'stopY') {
				...
				colision = true;
			} else if (this.RectCircleColliding(this.ball, wall) === 'stopX') {
				colision = true;
	...
	RectCircleColliding: function(ball, wall) {
		...
		if (distX > wall.w / 2 + ball.r) {
			return false;
		}
		if (distY > wall.h / 2 + ball.r) {
			return false;
		}
